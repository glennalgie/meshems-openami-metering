"""Generate the S1 bench wiring diagram (PC via USB-TTL, ESP32 5V pull-up rail)."""
import matplotlib, os
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from matplotlib.patches import FancyBboxPatch

C_12V="#8f1d1d"; C_5V="#d83a3f"; C_GND="#2c3b43"; C_TX="#2f7fd6"; C_RX="#c98a00"
...
def resistor(x,y0,y1,label,ldx=2.6):
    ...
    ax.plot(xs,ys,color=C_5V,lw=1.8)
    ax.text(x+ldx,(y0+y1)/2,label,fontsize=7.5,color=C_5V,va="center",
            ha=("left" if ldx>0 else "right"))

# components
box(4,20,17,15,"12 V PSU","limit 50-100 mA")
box(38,17,19,20,"IVY MD0630","")
box(70,17,24,17,"USB-TTL (FTDI)","3 wires -> PC")
box(70,42,24,10,"ESP32-S3","USB wall charger")
...
# 5 V rail from ESP32 (kept clear of every box)
wire([(74,42),(74,40),(58,40)],C_5V,2.6)
ax.text(64,41.4,"+5 V rail  (pull-ups)",ha="center",fontsize=8,color=C_5V)

# data lines in the gap between module and FTDI (crossed)
wire([(57,30),(66,30),(66,27),(70,27)],C_TX,2.4)   # module TX -> FTDI RXD
wire([(70,23),(69,23),(69,25),(57,25)],C_RX,2.4)   # FTDI TXD -> module RX
ax.text(58.5,33.4,"TX -> RXD",ha="left",fontsize=7.5,color=C_TX)
ax.text(58.5,21.8,"RX <- TXD",ha="left",fontsize=7.5,color=C_RX)

# pull-ups tapping each data line, up to the 5 V rail
resistor(60,40,30,"4.7k",ldx=2.4)
resistor(67,40,25,"4.7k",ldx=-2.4)
dot(60,30,C_5V); dot(67,25,C_5V)

# common ground
wire([(12,13),(88,13)],C_GND,2.6)
for x in (12,47,82,88): dot(x,13,C_GND)
wire([(12,20),(12,13)],C_GND,2.2)   # PSU -
wire([(47,17),(47,13)],C_GND,2.2)   # module GND
wire([(82,17),(82,13)],C_GND,2.2)   # FTDI GND
wire([(88,42),(88,13)],C_GND,2.2)   # ESP32 GND
ax.text(50,10.4,"COMMON GROUND   —   PSU(-)  ·  module  ·  USB-TTL  ·  ESP32",
...
ax.text(50,53.5,"S1 bench: PC reads the MD0630 through the USB-TTL; the ESP32 5 V feeds the pull-ups",
        ha="center",fontsize=9.2,color=INK)
plt.tight_layout()
out=os.path.join(os.path.dirname(__file__),"wiring_S1.png")
