---
title: "S1 & S2 — IVY MD0630 Leakage Sensor: Modbus Bring-up, Register Map & Live ESP32 Read"
...
**Result: S1 and S2 are both complete.** The module communicates, the full register map has been
derived from the live device and confirmed against IVY's own PC tool (`CT.exe`), and the ESP32 driver
now reads the live module **reliably over its own RS-485/UART bus** (Section on S2).
...
## S2 — Live read on the ESP32

With the register map known, the module was moved from the PC USB-TTL onto the **ESP32's own
RS-485/UART bus** and the `Modbus_MD0630` driver was switched from mock to **real reads**.
...
The MD0630 is UART, so it connects **directly** to the ESP32 UART pins (no RS-485 transceiver).
Pull-ups go to **3V3** — the ESP32-S3 GPIOs are **not** 5 V tolerant. The pins are the **GPIO numbers
42 / 7**, not the silkscreen "RX"/"TX" (which are the USB console UART, GPIO44/43).

### Evidence — serial capture (module at rest)
...
**12 consecutive successful reads, 0 failures.** The driver reads `0x0000`/`0x0001` at FC 0x03, scales
×0.1 mA, and feeds `leakageModel` (published on `subpanel_RCMleaks`).

### Two gotchas resolved

- **Wrong pins:** the module was first on the silkscreen "RX/TX" (USB console, GPIO44/43) instead of
  **GPIO42/GPIO7** → timeouts; moving to the numbered pins gave the first successful read.
- **Address collision:** the SHT20 is polled at address 1 — the same as the CT during bring-up →
  intermittent reads. A `LEAKAGE_BRINGUP` build flag skips the SHT20 poll → reliable reads. In
  production the CT is commissioned to node 100, so both coexist.
...
1. **S3 — threshold writes:** write `0x0002` (DC) / `0x0003` (AC) via **FC 0x06**, value = mA × 10.
   ...
   `CT.exe` write), and **always read back to confirm** (per Glenn). Default life-safety values:
   DC **6 mA**, AC **30 mA**.
2. **Confirm DC-vs-AC** of `0x0000`/`0x0001` by inducing a known leak.
